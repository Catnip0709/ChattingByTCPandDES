...
2、逆初始置换IP
3、16轮迭代
在每轮迭代时，右边的数据段要经过的加密运算包括选择扩展运算E、密钥加运算、选择压缩运算S、置换P，合称f函数。
4、子秘钥生成
*/
...
private:
   vector<vector<bool>> subKey; // 16个子秘钥

   vector<bool> InitReplacementIP(vector<bool> input, int type); //初始/逆初始置换IP
   
   vector<bool> EBox(vector<bool> input); // f: 选择扩展运算E
   vector<bool> keyAddition(vector<bool> input, vector<bool> key); // f: 密钥加运算
   vector<bool> selectCompressionOperation(vector<bool> input); // f: 选择压缩运算S
   vector<bool> replacementOp(vector<bool> input); // f: 置换运算P

   int MakeKey(vector<bool> initKey); // 生成16个密钥中的每一个子密钥，得到subKey
